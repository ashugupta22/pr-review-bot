    ...
    try {
      ...
      for (const chunk of chunks) {
        try {
          ...
          openaiErrors++;
          console.error(`[error] OpenAI reviewChunk failed (chunk ${chunks.indexOf(chunk) + 1}/${chunks.length}):`, e.message);
          // Continue with other chunks but track errors
        }
      }

      // Fail if all chunks failed
      if (openaiErrors === chunks.length && chunks.length > 0) {
        throw new Error(`All OpenAI API calls failed (${openaiErrors}/${chunks.length} chunks)`);
      }
      ...
      for (let i = 0; i < reviewComments.length; i += batchSize) {
        ...
        try {
          ...
          throw new Error(`Failed to post review comments: ${e.message}`);
        }
      }
    } catch (err) {
      console.error('[error] Review engine failed:', err.message);
